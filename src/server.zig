...
const Stats = struct {
    // Use usize so atomics map to the target's native word size; 32-bit
    // targets (e.g. armv7) lack lock-free 64-bit atomics. Connection counters
    // never need more than the native word width for an observability hint.
